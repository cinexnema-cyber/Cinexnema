...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    if (error) {
      console.error("❌ Erro nos parâmetros:", error, errorDescription);
      setError(
        errorDescription || "Link de redefinição inválido ou expirado. Solicite um novo link.",
      );
      setTokenValid(false);
      return;
    }

    // Aceita links com access_token direto OU com code (PKCE)
    if (!accessToken && !code) {
      setError(
    ...
    if (type && !['recovery', 'magiclink', 'password_recovery'].includes(type)) {
      console.warn("⚠️ Tipo de link não reconhecido:", type, "- mas continuando...");
    }

    // Set the session with the tokens or exchange code for session
      ...
      try {
        ...
        if (code && !accessToken) {
          try {
            ...
            console.log("✅ Sessão criada via code");
            setTokenValid(true);
            ...
            console.error("❌ Falha ao trocar código por sessão:", codeErr);
        ...
        // Tentar com refresh_token se disponível, senão apenas access_token
        const sessionData = {
        ...
        if (error) {
          ...
          // Tentar método alternativo para reset de senha
          try {
            ...
            // Se ainda não funcionou, mostrar erro
            setError(
          ...
          if (data.user) {
            ...
            if (!profileError && userProfile) {
              ...
              console.log("⚠️ Perfil não encontrado no banco, mas token é válido");
            }
          }
        }
      } catch (err) {
