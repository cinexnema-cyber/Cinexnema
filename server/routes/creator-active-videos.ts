...
// Unified: list creator videos from Supabase and enrich approved ones with api.video analytics
export async function getCreatorActiveApprovedVideos(req: Request, res: Response) {
  try {
    ...
    const videos = await Promise.all((data || []).map(async (row: any) => {
      ...
      if (token && vodId && String(row.status) === "approved") {
        try {
          ...
          analytics = { error: "Falha ao carregar analytics" };
        }
      }
    ...
    return res.json({ success: true, videos, period });
  } catch (e: any) {
    console.error("creator.active-videos error:", e?.message || e);
